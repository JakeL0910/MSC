...
//
// Mirrors the rest of MLC's "works in dev, real service in prod" approach:
...
export async function sendEmail(to: string, subject: string, html: string): Promise<{ delivered: boolean }> {
  ...
  const from = process.env.MAIL_FROM || 'MLC <onboarding@resend.dev>'

  if (!key) {
    console.log(`\n[MLC] Email to ${to} — "${subject}" (no mail provider configured; not sent)\n`)
...
export async function sendMagicLink(email: string, link: string): Promise<{ delivered: boolean }> {
  ...
  return sendEmail(email, 'Your MLC sign-in link', html)
}
